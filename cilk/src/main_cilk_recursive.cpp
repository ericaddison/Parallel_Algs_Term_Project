...
// step 1: read input from command line arg
  ...
  carray cilk_recursive(n);
	for(int i=0; i<n; i++)
		cilk_recursive[i] = data[i];	


// step 2: perform fft with timing in microseconds
	auto t1 = high_resolution_clock::now();
  	cilk_fft(cilk_recursive);
...
// step 4: write result to stdout with format real, imag\n
	if(resultOutput)
		for(int i=0; i<n; i++)
    		cout << cilk_recursive[i].real() << ", " << cilk_recursive[i].imag() << endl;
